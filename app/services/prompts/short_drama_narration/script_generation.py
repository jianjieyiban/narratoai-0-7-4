...
### 1. 黄金开场（3秒法则 - 参考"小片片说剧"的高能开场）
**高能开场定生死，47%的用户3秒内决定是否划走**
- **名场面开场**（推荐）：短剧开头必须出现具有视觉冲击力的名场面
  * 古装剧：用耳光/跳崖镜头
  * 甜宠剧：选壁咚/修罗场片段
  * 悬疑剧：聚焦凶案特写
  * 技巧：截取主角情绪爆发的0.5秒动态帧，搭配"这绝对是我见过最XXX的XXX"句式
- **悬念设置**：直接抛出最核心的冲突或疑问
...
### 2. 主线提炼（去油式叙事 - 参考"小片片说剧"的成功模式）
**采用"去油式叙事结构"，直接进入内容，不做自我介绍和过场动画**
- **汉堡包结构**（推荐）：高能片段5秒 + 背景铺垫15秒 + 悬念收尾5秒，平均完播率可达65%以上
- 快节奏解说，速度超越原剧，专注核心主线
...
**考虑文案与画面、音乐的完美结合**
- **动态字幕是刚需**：重点关键词加大加粗变色
- **"标重点"大法**：说到具体物件时，用圆圈、箭头高亮标出
- **画面&解说同步**：嘴里说"把鸡蛋打散"，画面就得是蛋液在碗里搅动
- 在情感高潮处预设BGM卡点
- 解说节奏要配合画面节奏
- 重要台词处保留原声，解说适时停顿
- 追求文案+画面+音乐的协同效应
...
### 解说文案要求（自媒体平台标准）：
- **字数控制**：根据时间戳动态计算，每0.3-0.35秒一个字（约每分钟200字语速），每段解说文案控制在60-120字（对应约20-40秒片段）
- **单句长度**：控制在12-18字，避免超长句子，多用短句
- **语言风格**：生动有趣，富有感染力，符合短视频观众喜好，必须口语化
  * 参考风格："身为一个名声恶臭的政客，他知道自己早晚会被暗杀"
  * 直接定性，制造紧张感和代入感
  * 多用"这个"、"那个"、"就"、"其实"、"真的"、"太XX了"等口语词
- **情感调动**：能够有效调动观众情绪，产生代入感
  * 使用"而这时"、"没想到"、"原来"、"突然"等转折词增强戏剧性
- **节奏把控**：快节奏但不失条理，紧凑但不混乱，每分钟200-250字语速
  * 短句推进剧情，长句渲染氛围，但长句不超过25字
- **信息密度**：每3-5秒一个信息点，避免冗余，保持新鲜感
...
2. **严格基于提供的剧情和字幕**，不虚构内容，所有解说必须与剧情和字幕内容匹配
3. **画面与解说必须完全匹配**：
   - 如果提供的剧情和字幕是短剧内容，必须生成短剧解说
   - 如果剧情描述的是非短剧内容（如生活场景、动物世界等），必须基于实际内容生成对应的解说，不能强行生成短剧解说
   - picture字段必须准确描述画面内容，narration必须与picture完全匹配
4. **突出核心冲突**，舍弃无关细节
5. **强化观众体验**，始终考虑观看感受
...
7. **融入经典解说技巧**：
   ...
   - 运用悬念设置和反转技巧
   - 保持强烈的画面感和代入感
8. **验证检查**：生成后检查每个片段的narration是否与对应的picture描述的内容匹配，如果出现不匹配，必须重新生成
