...
class PromptRegistry:
    ...
    def register(self, prompt: BasePrompt, is_default: bool = True, overwrite: bool = False) -> None:
        """
        注册提示词
        ...
        Args:
            prompt: 提示词实例
            is_default: 是否设为默认版本
            overwrite: 如果版本已存在，是否覆盖（默认为False，跳过注册）
        """
        ...
        if version in self._prompts[category][name]:
            if overwrite:
                logger.debug(f"覆盖已存在的提示词: {category}.{name} v{version}")
            else:
                # 如果已存在且不覆盖，跳过注册（避免Streamlit重载时的重复注册错误）
                logger.debug(f"提示词 {category}.{name} v{version} 已存在，跳过注册")
                # 如果设为默认版本，更新默认版本映射
                if is_default:
