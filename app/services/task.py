...
def _save_script_if_possible(script_path: str, script_items: list) -> None:
    """将更新后的脚本写回原始文件，确保后续查看与实际剪辑一致。"""
    if not script_path:
        return

    normalized_path = script_path.replace("\\", "/").lower()
    if "/templates/" in normalized_path:
        logger.info(f"跳过更新模板脚本: {script_path}")
        return

    try:
        os.makedirs(os.path.dirname(script_path), exist_ok=True)
        with open(script_path, "w", encoding="utf-8") as script_file:
            json.dump(script_items, script_file, ensure_ascii=False, indent=2)
        logger.info(f"已将更新后的脚本写回: {script_path}")
    except Exception as e:
        logger.warning(f"更新脚本文件失败: {script_path}, 错误: {e}")
...
def start_subclip(task_id: str, params: VideoClipParams, subclip_path_videos: dict = None):
    ...
    _save_script_if_possible(params.video_clip_json_path, new_script_list)
    _save_script_if_possible(params.video_clip_json_path, new_script_list)
