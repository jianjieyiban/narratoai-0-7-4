...
def check_format(script_content: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, clip in enumerate(data):
            ...
            # 检查 narration 字段是否包含占位符（模板文件的示例文字）
            # 使用更精确的检测，避免误报
            narration_value = clip['narration'].strip()
            # 检测明确的模板占位符模式（更全面的检测）
            narration_placeholder_patterns = [
                '解说文案示例',
                '解说文案示例：',
                '解说文案示例:',
                '示例：',
            ...
            # 也检测包含这些关键词的文本（不仅仅是以其开头）
            narration_placeholder_keywords = [
                '解说文案示例',
                '示例:在遥远',
                '示例：在遥远',
            ]
            # 如果narration以这些占位符开头，或者包含占位符关键词，很可能是模板文件
            starts_with_placeholder = any(narration_value.startswith(pattern) for pattern in narration_placeholder_patterns)
            contains_placeholder = any(keyword in narration_value for keyword in narration_placeholder_keywords)
            if starts_with_placeholder or contains_placeholder:
                return {
                    'success': False,
                    'message': f'第{i+1}个片段的narration包含占位符文字，请使用AI生成真实解说文案',
                    'details': f'检测到占位符: "{narration_value}"\n\n提示：模板文件包含示例文字，必须先使用"AI生成画面解说脚本"功能生成真实脚本后才能生成视频。'
                }

            # 检查 picture 字段是否包含占位符
            picture_value = clip.get('picture', '').strip()
            picture_placeholder_patterns = [
                '画面描述示例',
                '画面描述示例：',
                '画面描述示例:',
                '画面描述示例：在',
            ...
            # 如果picture以这些占位符开头，很可能是模板文件
            if any(picture_value.startswith(pattern) for pattern in picture_placeholder_patterns):
                return {
                    'success': False,
                    'message': f'第{i+1}个片段的picture包含占位符文字，请使用AI生成真实画面描述',
                    'details': f'检测到占位符: "{picture_value}"\n\n提示：模板文件包含示例文字，必须先使用"AI生成画面解说脚本"功能生成真实脚本后才能生成视频。'
                }
