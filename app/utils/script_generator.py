...
记住：要敢于用"温和的违反"制造笑点，但要把握好尺度，让观众在轻松愉快中感受到乐趣。"""
    ...
    def calculate_duration_and_word_count(self, time_range: str) -> int:
        ...
        try:
            ...
            # 根据自媒体平台标准计算字数: 每分钟200-220字语速，约每0.3秒一个字
            # 自媒体视频通常语速较快，需要信息密度高
            # 使用0.32秒/字（对应每分钟约187字），确保节奏紧凑但不失清晰度
            word_count = int(duration / 0.32)
            
            # 确保字数在合理范围内
            word_count = max(8, min(word_count, 200))  # 限制在8-200字之间，避免单片段过长
