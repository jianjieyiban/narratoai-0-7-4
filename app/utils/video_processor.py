...
class VideoProcessor:
    def __init__(self, video_path: str):
        ...
        # 安全地获取视频信息，确保所有值都是有效的数字
        fps_value = self.video_info.get('fps', '25')
        duration_value = self.video_info.get('duration', '0')
        width_value = self.video_info.get('width', '1280')
        height_value = self.video_info.get('height', '720')
        
        # 确保值不是 None，并转换为浮点数/整数
        try:
            self.fps = float(fps_value) if fps_value is not None else 25.0
        except (ValueError, TypeError):
            self.fps = 25.0
            
        try:
            self.duration = float(duration_value) if duration_value is not None else 0.0
        except (ValueError, TypeError):
            self.duration = 0.0
            
        try:
            self.width = int(width_value) if width_value is not None else 1280
        except (ValueError, TypeError):
            self.width = 1280
            
        try:
            self.height = int(height_value) if height_value is not None else 720
        except (ValueError, TypeError):
            self.height = 720
        
        # 确保 fps 和 duration 都不为 0 或 None
        if self.fps <= 0:
            self.fps = 25.0
        if self.duration <= 0:
            logger.warning(f"视频时长无效或为0，尝试从视频文件重新获取")
            # 尝试使用 ffprobe 重新获取时长
            try:
                import subprocess
                cmd = [
                ...
                if duration_str:
                    ...
                    logger.info(f"成功从视频文件获取时长: {self.duration:.2f}秒")
            except Exception as e:
                logger.error(f"无法获取视频时长: {e}")
                raise ValueError(f"无法获取视频时长信息，请检查视频文件是否损坏: {self.video_path}")
    ...
    def extract_frames_by_interval(self, output_dir: str, interval_seconds: float = 5.0,
        ...
        # 确保 interval_seconds 是有效的数字
        if interval_seconds is None:
            interval_seconds = 5.0
            logger.warning("帧间隔为 None，使用默认值 5.0 秒")
        try:
            interval_seconds = float(interval_seconds)
            if interval_seconds <= 0:
                interval_seconds = 5.0
                logger.warning(f"帧间隔无效，使用默认值 5.0 秒")
        except (ValueError, TypeError):
            interval_seconds = 5.0
            logger.warning(f"帧间隔格式错误，使用默认值 5.0 秒")
        ...
        if end_time is None or end_time <= 0:
            logger.error(f"视频时长无效: {end_time}")
            raise ValueError(f"视频时长无效，无法提取关键帧。请检查视频文件: {self.video_path}")
        
        # 确保 fps 是有效的数字
        if self.fps is None or self.fps <= 0:
            logger.error(f"视频帧率无效: {self.fps}")
            raise ValueError(f"视频帧率无效，无法提取关键帧。请检查视频文件: {self.video_path}")
        ...
        with tqdm(total=len(extraction_times), desc="🎬 提取视频帧", unit="帧",
            ...
            for i, timestamp in enumerate(extraction_times):
                # 确保 timestamp 和 self.fps 都是有效的数字
                if timestamp is None or self.fps is None:
                    logger.error(f"时间戳或帧率无效: timestamp={timestamp}, fps={self.fps}")
                    continue
